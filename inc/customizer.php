...
function blover_customize_register( $wp_customize ) {
	...
	$wp_customize->add_setting( 'home_page_display_content', array(
		'default'        => 1,
				'sanitize_callback' => 'wp_validate_boolean',
		) );

	$wp_customize->add_control( 'home_page_display_content', array(
		'label'   => esc_html__( 'Display Content on Home and Archive Pages.', 'blover' ),
		'section' => 'home_page',
		'type'    => 'checkbox',
		) );

	$wp_customize->add_setting( 'home_page_show_featured_images', array(
		'default'        => 1,
				'sanitize_callback' => 'wp_validate_boolean',
		) );

	$wp_customize->add_control( 'home_page_show_featured_images', array(
		...
		$wp_customize->add_control( 'hide_meta_on_home_archive', array(
			...
			$wp_customize->add_control( 'pagination', array(
				...
				$wp_customize->add_setting( 'wpp_img_size', array(
				'default'        => 'medium',
						'sanitize_callback' => 'blover_sanitize_select_img_size',
				) );

			$wp_customize->add_control( 'wpp_img_size', array(
	...
	$wp_customize->add_control( 'single_post_show_featured_image', array(
		'label'   => esc_html__( 'Show Featured Images In Standard Posts', 'blover' ),
		'section' => 'single_page',
		'type'    => 'checkbox',
		) );

		$wp_customize->add_setting( 'single_page_related_posts_show', array(
			'default'        => 1,
					'sanitize_callback' => 'wp_validate_boolean',
			) );

		$wp_customize->add_control( 'single_page_related_posts_show', array(
			'label'   => esc_html__( 'Show Related Posts.', 'blover' ),
			'section' => 'single_page',
			'type'    => 'checkbox',
			) );

			$wp_customize->add_setting( 'single_page_related_posts_title', array(
			'default'        => esc_html__( 'You May Also Like', 'blover' ),
					'sanitize_callback' => 'esc_html',
			) );

			$wp_customize->add_control( 'single_page_related_posts_title', array(
		...
		$wp_customize->add_control( 'single_post_navigation', array(
			...
			$wp_customize->add_setting( 'single_page_related_posts_by_tag_or_cat', array(
				'default'        => 1,
						'sanitize_callback' => 'wp_validate_boolean',
				) );

			$wp_customize->add_control( 'single_page_related_posts_by_tag_or_cat', array(
		...
		$wp_customize->add_section( 'footer', array(
			'title'          => esc_html__( 'Footer', 'blover' ),
				'priority'       => 1030,
		) );

		$wp_customize->add_setting( 'footer_text', array(
		'default'        => '<p>Proudly powered by WordPress<span class="sep"> | </span>Theme: blover</p>',
				'sanitize_callback' => 'wp_kses_post',
		) );

		$wp_customize->add_control( 'footer_text', array(
		'label'   => esc_html__( 'Footer Text', 'blover' ),
		'section' => 'footer',
		'type'    => 'textarea',
		) );
		...
		$wp_customize->add_setting( 'woocommerce_sidebar', array(
			'default'        => 1,
					'sanitize_callback' => 'wp_validate_boolean',
			) );

		$wp_customize->add_control( 'woocommerce_sidebar', array(
			'label'   => esc_html__( 'Enable Sidebar on Shop Page', 'blover' ),
			'section' => 'woocommerce_settings',
			'type'    => 'checkbox',
			) );

			$wp_customize->add_setting( 'woocommerce_show_page_title', array(
				'default'        => 0,
						'sanitize_callback' => 'wp_validate_boolean',
				) );

			$wp_customize->add_control( 'woocommerce_show_page_title', array(
